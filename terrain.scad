...
module terrain(ul, ur, ll, lr, l) {
  ...
  if(l <= levels) {
    ...
    t2y = ul.y;
    t2z = (ul.z + ur.z) / 2 +           rand1(t2x - ul.x) / 2;
    l2x = ul.x;
    l2y = ul.y + (ll.y - ul.y) / 2;
    l2z = (ul.z + ll.z) / 2 +           rand1(ll.y - l2y) / 2;
    r2x = ur.x;
    r2y = ur.y + (lr.y - ur.y) / 2;
    r2z = (ur.z + lr.z) / 2 +           rand1(lr.y - r2y) / 2;
    b2x = ll.x + (lr.x - ll.x) / 2;
    b2y = ll.y;
    b2z = (ll.z + lr.z) / 2 +           rand1(lr.x - b2x) / 2;
    m2x = b2x;
    m2y = r2y;
    m2z = (ul.z + ur.z + ll.z + lr.z) / 4; // randomize?
